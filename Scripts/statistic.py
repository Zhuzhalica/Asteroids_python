...
class Stat:
    def __init__(self, screen=pygame.display.set_mode((800, 450)),
                 space_ship=None, player_name='', save_path='save.txt'):
        ...
        if space_ship is not None:
            ...
            self.stat_image = self.font.render(str(self.score), True,
    ...
    def parse_save(self):
        """Достать статистику из сохранения"""
    ...
    def make_stat(self, enemy):
        ...
        if type(enemy) is Asteroid:
            self.score += int(100 *
                              (enemy.image.get_size()[0] / enemy.max_size))
            enemy.death()
    ...
    def draw_stat(self):
        ...
        self.stat_image = self.font.render(f"Score: {str(self.score)}",
